...
print(os.system('ncdump -c /Users/jun-hyeokson/research/climate_indices/program/00_make_nc/NINO3.4.nc'))
...
data = vel_pot.reshape(44,12,73,144)
...
for mon in range(0,12):
  ave[mon] = np.mean(index[:,mon])
  std[mon] = np.std(index[:,mon])
...
for mon in range(0,12):
  ave[mon] = np.mean(nino[:,mon])
  std[mon] = np.std(nino[:,mon])

for mon in range(0,12):
  nino[:,mon] = (nino[:,mon]-ave[mon])/std[mon]

temp_index = np.zeros((43),dtype=float)
...
index_temp = np.zeros((26),dtype=float)
nino_temp = np.zeros((26),dtype=float)
temp_num = 0
...
for t in range(13):
  print('moving',t,np.corrcoef(nino[0+t:30+t,7-1],index[0+t:30+t,7-1]))

#-write indices to netcdf
try: ncfile.close()  # just to be safe, make sure dataset is not already open.
except: pass
ncfile = Dataset('./final_index.nc',mode='w',format='NETCDF4_CLASSIC')
print(ncfile)

time_dim = ncfile.createDimension('time', None) # unlimited axis (can be appended to).

ncfile.title='final_index 1979~2020'
